...
public class LDAP {
  ...
  public func login(info: Login, completion: @escaping (String?)->Void) {
    ...
    /**
     Cancels an ongoing LDAP search operation.

     When called, this method cancels the current LDAP search operation that was initiated using the `connect` method. It sends an abandon request to the LDAP server, effectively terminating the search operation.

     - Note: This method should be called if you need to cancel a long-running LDAP search to free up resources and stop waiting for the search results.

     - Warning: Ensure that the `connect` method has been previously called to establish an LDAP connection before using this method.

    */
     
    public func cancel(){
        
        let servctrl = UnsafeMutablePointer<UnsafeMutablePointer<LDAPControl>?>(bitPattern: 0)
        let clientctr = UnsafeMutablePointer<UnsafeMutablePointer<LDAPControl>?>(bitPattern: 0)
        
        ldap_abandon_ext(ldap, 0, servctrl, clientctr)
    ...
    public var dictionary:[String:[String:Any]] { get {
      ...
      for aset in _attr {
        ...
        for a in aset.attributes {
            ...
            guard let value = a.values.first else {
                continue
            }
  ...
  String:Any]] {
    ...
    let r = withCArrayOfString(array: attributes) { pAttribute -> Int32 in
        ...
        self.cancel()
      // perform the search
        let result = ldap_search_ext_s(self.ldap, base, scope.rawValue, filter, pAttribute, 0, &serverControl, nil, nil, Int32(self.limitation), &msg)
